...
public record CouponCampaignResponse(
    ...
    public static CouponCampaignResponse summary(CouponCampaign campaign, Instant now) {
    ...
    private static Integer remaining(Integer issueLimit, int issuedCount) {
    ...
    public record StoreSummary(Long id, String publicName) {
        static StoreSummary from(Store store) {
            return store == null ? null : new StoreSummary(store.getId(), store.getPublicName());
        }
    }
}
