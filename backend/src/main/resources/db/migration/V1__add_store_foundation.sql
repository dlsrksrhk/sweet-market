...
CREATE TABLE IF NOT EXISTS store_memberships (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_store_memberships_store_member UNIQUE (store_id, member_id),
    CONSTRAINT chk_store_memberships_role CHECK (role IN ('OWNER', 'MANAGER'))
);
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_store_memberships_store') THEN
