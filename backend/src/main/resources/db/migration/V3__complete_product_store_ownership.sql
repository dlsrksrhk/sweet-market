...
END IF;
    ...
    IF
EXISTS (SELECT 1 FROM products WHERE store_id IS NULL) THEN
...
END IF;
    ...
    IF
to_regclass('orders') IS NOT NULL THEN
UPDATE orders o
SET seller_id = s.owner_member_id FROM products p
        JOIN stores s
ON s.id = p.store_id
...
IF
EXISTS (SELECT 1 FROM orders WHERE seller_id IS NULL) THEN
            RAISE EXCEPTION 'Cannot migrate orders: seller_id contains null values';
END IF;

        IF
NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_orders_seller') THEN
ALTER TABLE orders
