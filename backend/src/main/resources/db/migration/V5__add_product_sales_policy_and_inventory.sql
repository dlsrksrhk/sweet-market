...
BEGIN
    ...
    ALTER TABLE products ADD COLUMN IF NOT EXISTS sales_policy VARCHAR(20);
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_products_sales_policy') THEN
        ALTER TABLE products
            ADD CONSTRAINT chk_products_sales_policy
            CHECK (sales_policy IN ('SINGLE_ITEM', 'STOCK_MANAGED'));
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_products_low_stock_threshold') THEN
        ALTER TABLE products
    ...
    CREATE TABLE IF NOT EXISTS inventories (
        ...
        CONSTRAINT chk_inventories_total_quantity_non_negative CHECK (total_quantity >= 0),
        CONSTRAINT chk_inventories_reserved_quantity_non_negative CHECK (reserved_quantity >= 0),
        CONSTRAINT chk_inventories_reserved_quantity_within_total CHECK (reserved_quantity <= total_quantity)
    );
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_inventories_product') THEN
        ALTER TABLE inventories
            ADD CONSTRAINT fk_inventories_product
            FOREIGN KEY (product_id) REFERENCES products (id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_inventory_adjustments_inventory') THEN
    ...
    IF to_regclass('members') IS NOT NULL
