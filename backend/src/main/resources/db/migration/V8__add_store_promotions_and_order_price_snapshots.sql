...
BEGIN
    IF
to_regclass('public.orders') IS NULL THEN
        RETURN;
END IF;
...
ALTER TABLE orders
    ADD COLUMN promotion_campaign_id BIGINT;
ALTER TABLE orders
    ADD COLUMN promotion_discount_amount BIGINT;
ALTER TABLE orders
...
SET list_price                = p.price,
    ...
    final_price               = p.price FROM products p
WHERE o.product_id = p.id;

ALTER TABLE orders
    ALTER COLUMN list_price SET NOT NULL;
ALTER TABLE orders
    ALTER COLUMN promotion_discount_amount SET NOT NULL;
ALTER TABLE orders
