...
export function AdminOperationsDashboardPage() {
  ...
  const provenanceUnavailable = !provenancePending && coverage === undefined;
  const drilldownsEnabled = coverage === 'PARTIAL' || coverage === 'TRACKED';
  ...
  return (
    <main className="operations-dashboard admin-operations-dashboard">
      ...
      <section className="operations-dashboard-panel" aria-labelledby="admin-campaign-outcome-title">
        ...
        <DrilldownBlock title="캠페인 성과" query={campaignsQuery} empty="해당 조건의 캠페인 성과가 없습니다." unmeasured={periodUnmeasured} provenancePending={provenancePending} provenanceUnavailable={provenanceUnavailable}><CampaignTable page={campaignsQuery.data} /></DrilldownBlock>
        <PageNavigation page={urlState.pages.campaigns} data={drilldownsEnabled ? campaignsQuery.data : undefined} label="캠페인 성과 페이지 이동" onChange={(campaigns) => updateUrlState((next) => { next.pages.campaigns = campaigns; })} />
        <AggregateProvenance dashboard={dashboardQuery.data} period={period} />
        <DrilldownBlock title="구매·발급 결과" query={outcomesQuery} empty="해당 조건의 결과 지표가 없습니다." disabled={productInputInvalid} unmeasured={periodUnmeasured} provenancePending={provenancePending} provenanceUnavailable={provenanceUnavailable}><OutcomeTable page={outcomesQuery.data} /></DrilldownBlock>
        <PageNavigation page={urlState.pages.outcomes} data={drilldownsEnabled && !productInputInvalid ? outcomesQuery.data : undefined} label="결과 지표 페이지 이동" onChange={(outcomes) => updateUrlState((next) => { next.pages.outcomes = outcomes; })} />
      </section>
      ...
      <section className="operations-dashboard-panel" aria-labelledby="admin-inventory-title">
        ...
        <DrilldownBlock query={inventoryQuery} empty="해당 조건의 재고 압력 지표가 없습니다." disabled={productInputInvalid} unmeasured={periodUnmeasured} provenancePending={provenancePending} provenanceUnavailable={provenanceUnavailable}><InventoryTable page={inventoryQuery.data} /></DrilldownBlock>
        <PageNavigation page={urlState.pages.inventory} data={drilldownsEnabled && !productInputInvalid ? inventoryQuery.data : undefined} label="재고 압력 페이지 이동" onChange={(inventory) => updateUrlState((next) => { next.pages.inventory = inventory; })} />
      </section>
      ...
      <section className="operations-dashboard-panel" aria-labelledby="admin-audit-title">
        ...
        <AggregateProvenance dashboard={dashboardQuery.data} period={period} />
        <DrilldownBlock query={auditsQuery} empty="해당 조건의 캠페인 감사 기록이 없습니다." unmeasured={periodUnmeasured} provenancePending={provenancePending} provenanceUnavailable={provenanceUnavailable}><AuditTable page={auditsQuery.data} /></DrilldownBlock>
        <PageNavigation page={urlState.pages.audits} data={drilldownsEnabled ? auditsQuery.data : undefined} label="캠페인 감사 페이지 이동" onChange={(audits) => updateUrlState((next) => { next.pages.audits = audits; })} />
      </section>
...
function DrilldownBlock({ title, query, empty, disabled = false, unmeasured = false, provenancePending = false, provenanceUnavailable = false, children }: { title?: string; query: { isLoading: boolean; error: unknown; data?: Page<unknown> }; empty: string; disabled?: boolean; unmeasured?: boolean; provenancePending?: boolean; provenanceUnavailable?: boolean; children: ReactNode }) {
  const ready = !disabled && !provenancePending && !provenanceUnavailable && !unmeasured;
  return <section className="admin-drilldown-block">{title ? <h3>{title}</h3> : null}{disabled ? <EmptyState title="올바른 상품 ID를 입력해주세요" description="필터를 수정할 때까지 기존 조건을 넓혀 조회하지 않습니다." /> : null}{!disabled && provenancePending ? <p className="status-text" role="status">추적 범위를 확인하고 있습니다.</p> : null}{!disabled && !provenancePending && provenanceUnavailable ? <EmptyState title="추적 범위를 확인할 수 없습니다." description="플랫폼 운영 요약을 다시 불러온 뒤 상세 지표를 확인해주세요." /> : null}{!disabled && !provenancePending && !provenanceUnavailable && unmeasured ? <EmptyState title="측정 전" description="선택한 기간은 운영 이벤트 추적 시작 전이므로 0으로 해석하지 않습니다." /> : null}{ready && query.isLoading ? <p className="status-text" role="status">상세 지표를 불러오고 있습니다.</p> : null}{ready && query.error ? <ErrorState message={errorMessage(query.error, '상세 지표를 불러오지 못했습니다.')} /> : null}{ready && query.data?.content.length === 0 ? <EmptyState title={empty} description="기간이나 필터를 바꿔 다시 확인해보세요." /> : null}{ready && query.data?.content.length ? children : null}</section>;
}
