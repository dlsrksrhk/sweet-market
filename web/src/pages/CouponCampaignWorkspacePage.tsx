...
export function CouponCampaignWorkspacePage() {
  ...
  return <main className="coupon-workspace">
    ...
    <section className="coupon-panel"><div className="coupon-panel-heading"><div><p className="eyebrow">CAMPAIGNS</p><h2>{store.publicName} 쿠폰</h2></div><button className="text-button" type="button" onClick={() => setShowForm((value) => !value)}>{showForm ? '등록 닫기' : '쿠폰 등록'}</button></div>
      ...
      <div className="coupon-list">{campaignsQuery.data?.content.map((campaign) => <article className="coupon-card" key={campaign.id}><div><StatusBadge status={campaign.effectiveStatus} /><h3>{campaign.title}</h3><p>{discountText(campaign)} · 최소 {money(campaign.minimumPurchaseAmount)}원</p><span>발급 {formatDate(campaign.issueStartsAt)} ~ {formatDate(campaign.issueEndsAt)}</span><span>{issueAvailabilityText(campaign)}</span></div><div className="coupon-card-actions"><span>{campaign.scope === 'ALL_PRODUCTS' ? '전체 상품' : `${campaign.targetCount}개 상품`}</span><Link className="text-button" to={`/me/store/coupons/${store.storeId}/${campaign.id}`}>상세·수정</Link></div></article>)}</div>
...
export function CouponCampaignForm({ products, initialValue, pending, disabled = false, error, submitLabel, onSubmit }: CouponCampaignFormProps) {
  ...
  return <form className="coupon-form" onSubmit={submit}>
    ...
    <label>발급 한도 (선택)<input disabled={disabled} type="number" min="1" value={issueLimit ?? ''} onChange={(event) => setIssueLimit(event.target.value === '' ? undefined : Number(event.target.value))} /></label>
...
export function issueAvailabilityText(campaign: { issueLimit: number | null | undefined; issuedCount: number; remainingIssueCount: number | null | undefined }) { return campaign.issueLimit == null ? '발급 무제한' : `발급 ${campaign.issuedCount} / ${campaign.issueLimit} · 잔여 ${campaign.remainingIssueCount}`; }
