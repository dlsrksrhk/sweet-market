...
export function StoreOperationsDashboardPage() {
  ...
  const purchaseOutcomesQuery = useQuery({ queryKey: storeOperationsDashboardQueryKeys.purchaseOutcomes(storeId, purchaseInput), queryFn: () => getStorePurchaseOutcomes(storeId, purchaseInput), enabled: drilldownsEnabled && urlState.tab === 'purchase-outcomes' });
  const campaignAuditsQuery = useQuery({ queryKey: storeOperationsDashboardQueryKeys.campaignAudits(storeId, auditsInput), queryFn: () => getStoreCampaignAudits(storeId, auditsInput), enabled: drilldownsEnabled && urlState.tab === 'campaign-audits' });
...
export function ProjectionFreshness({ generatedAt, projectionUpdatedAt, projectionLagSeconds, trackingStartedAt, coverage }: { generatedAt: string; projectionUpdatedAt: string | null; projectionLagSeconds: number; trackingStartedAt: string | null; coverage?: TrackingCoverage }) {
  if (coverage === 'UNTRACKED' && trackingStartedAt) return <div className="operations-freshness is-neutral" role="status"><strong>선택 기간은 추적 시작 전</strong><span>이 기간의 값은 측정된 0으로 해석하지 않습니다.</span></div>;
...
function LeadingFailureReasons({ reasons, coverage }: { reasons: { reason: string; count: number }[]; coverage: TrackingCoverage }) {
  return <section className="operations-reasons" aria-labelledby="operations-reasons-title"><h3 id="operations-reasons-title">주요 실패 사유</h3>{coverage === 'UNTRACKED' ? <p>선택 기간은 추적 시작 전이므로 실패 사유를 측정하지 않았습니다.</p> : reasons.length === 0 ? <p>{coverage === 'PARTIAL' ? '추적 시작 이후 측정된 실패 사유가 없습니다.' : '측정된 실패 사유가 없습니다.'}</p> : <ol>{reasons.map((reason) => <li key={reason.reason}><span>{reasonLabel(reason.reason)}</span><strong>{reason.count.toLocaleString('ko-KR')}건</strong></li>)}</ol>}</section>;
}
...
type DrilldownContentProps = { activeTab: DrilldownTab; store: OperableStore; coverage?: TrackingCoverage; provenancePending: boolean; campaigns?: Page<StoreCampaignMetric>; couponOutcomes?: Page<StoreCouponOutcome>; inventory?: Page<StoreInventoryPressure>; purchaseOutcomes?: Page<StorePurchaseOutcome>; audits?: Page<StoreCampaignAudit>; loading: boolean; error: unknown };
...
function DrilldownContent({ activeTab, store, coverage, provenancePending, campaigns, couponOutcomes, inventory, purchaseOutcomes, audits, loading, error }: DrilldownContentProps) {
  ...
  const partialNotice = coverage === 'PARTIAL' ? <p className="operations-provenance">추적 시작 이후 집계만 포함합니다.</p> : null;
  if (activeTab === 'campaigns' && campaigns) return <>{partialNotice}<CampaignTable page={campaigns} store={store} /></>;
  if (activeTab === 'coupon-outcomes' && couponOutcomes) return <>{partialNotice}<CouponOutcomeTable page={couponOutcomes} store={store} /></>;
  if (activeTab === 'inventory-pressure' && inventory) return <>{partialNotice}<InventoryTable page={inventory} /></>;
  if (activeTab === 'purchase-outcomes' && purchaseOutcomes) return <>{partialNotice}<PurchaseOutcomeTable page={purchaseOutcomes} /></>;
  if (activeTab === 'campaign-audits' && audits) return <>{partialNotice}<AuditTable page={audits} store={store} /></>;
  return null;
}
